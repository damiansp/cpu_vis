...
class FacialKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = os.path.join(
            self.root_dir, self.key_pts_frame.iloc[idx, 0])
...
class Normalize(object):
    '''Convert a color image to greyscale and normalize the color range to
    [0, 1].
    '''
...
class RandomCrop(object):
    ...
    Parameters
    - output_size (tuple or int): Desired output size. If int, square crop is
      made.
    '''
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image, key_pts = sample['image'], sample['keypoints']
        # if image has no greyscale color channel, add one
